...
@Tag(
    name = "Departments",
    description = "CRUD API для подразделений (иерархия + код 1:1 + тип)"
)
public interface DepartmentApiV1 {
  ...
  @Operation(
      ...
      description = "Создаёт подразделение. codeId обязателен (код 1:1). parentId/typeId опциональны."
  )
  @ApiResponse(
      responseCode = "201",
  ...
  @Operation(
      ...
      description = "Возвращает подразделение с раскрытыми связями (parent/code/type)."
  )
  @ApiResponse(
      responseCode = "200",
  ...
  @GetMapping("/{id}")
  ResponseEntity<DepartmentDto> getById(@PathVariable("id") UUID id);

  @Operation(
      summary = "Получить список подразделений (полная модель)",
      description = "Постраничный список подразделений с фильтрами. Возвращает раскрытые parent/code/type."
  )
  @ApiResponse(
      responseCode = "200",
      description = "Страница подразделений",
      content = @Content(schema = @Schema(implementation = Page.class))
  )
  @GetMapping
  ResponseEntity<Page<DepartmentDto>> getAll(
      @ParameterObject DepartmentFilters filters,
      @ParameterObject Pageable pageable);

  @Operation(
      summary = "Получить список подразделений (сокращённая модель)",
      description = "Постраничный список подразделений в сокращённом виде (для списков/таблиц)."
  )
  @ApiResponse(
      responseCode = "200",
      description = "Страница подразделений",
  ...
  @GetMapping("/short")
  ResponseEntity<Page<DepartmentShortDto>> getAllShort(
      @ParameterObject DepartmentFilters filters,
      @ParameterObject Pageable pageable);

  @Operation(
      summary = "Обновить подразделение",
      description = "Обновляет подразделение. Можно менять codeId (перепривязка к новому коду), parentId, typeId и поля."
  )
  @ApiResponse(
      responseCode = "200",
  ...
  @Operation(
      ...
      description = "Удаляет только подразделение (код/тип не удаляются). " +
