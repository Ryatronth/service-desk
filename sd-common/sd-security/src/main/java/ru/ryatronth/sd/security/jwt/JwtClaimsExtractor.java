...
public class JwtClaimsExtractor {
  ...
  public SdUserGroups userGroups(Jwt jwt) {
    ...
    try {
      ...
      log.warn("Не удалось прочитать claim '{}' из JWT. Группы будут пустыми. Причина: {}",
          properties.getClaims().getGroups(), e.getMessage());
      groups = List.of();
    }
    ...
    try {
      ...
      log.warn(
          "Не удалось сопоставить группы из JWT. Подразделение/рабочее место/роли будут пустыми. Причина: {}",
          e.getMessage());
      return new SdUserGroups(Set.of(), null, null);
    }
  }
