...
public final class ValueReaders {
    ...
    public static Optional<Boolean> bool(Map<String, ?> map, String key) {
        ...
        switch (raw) {
            ...
            case String s -> {
                ...
                boolean out =
                    v.equalsIgnoreCase("true") || v.equalsIgnoreCase("1") || v.equalsIgnoreCase("yes") || v.equalsIgnoreCase("y");
        ...
        log.debug("Не удалось привести значение к boolean: key={}, тип={}", key, raw.getClass().getName());
        return Optional.empty();
    }
    ...
    public static List<String> stringList(Map<String, ?> map, String key) {
        ...
        return col.stream().filter(Objects::nonNull).map(Object::toString).map(String::trim).filter(s -> !s.isEmpty()).toList();
    }
