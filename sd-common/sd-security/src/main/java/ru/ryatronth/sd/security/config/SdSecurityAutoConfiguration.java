...
public class SdSecurityAutoConfiguration {
    ...
    public UrlBasedCorsConfigurationSource corsConfigurationSource(SdSecurityProperties properties) {
        ...
        config.setExposedHeaders(List.of("Authorization", "Link", "X-Total-Count", "Content-Disposition"));
    ...
    @Bean
    @ConditionalOnMissingBean
    public KeycloakGroupPathClassifier keycloakGroupPathClassifier(SdKeycloakProperties keycloakProperties) {
    ...
    public Converter<Jwt, AbstractAuthenticationToken> jwtAuthenticationConverter(JwtClaimsExtractor claimsExtractor) {
    ...
    public SecurityUtils securityUtils(SdSecurityProperties properties, JwtClaimsExtractor jwtClaimsExtractor) {
        return properties.isEnabled() ? new JwtSecurityUtils(jwtClaimsExtractor) : new RandomSecurityUtils();
    }
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http,
        ...
        http.authorizeHttpRequests(auth -> auth.requestMatchers(permit).permitAll().anyRequest().authenticated())
