...
public class KeycloakAdminService {
  ...
  public List<KeycloakUserRepresentation> getUsersPage(int first, int max) {
    String url =
        sdKeycloakProperties.getBaseUrl() + "/admin/realms/" + sdKeycloakProperties.getRealm() +
            "/users?first=" + first
            + "&max=" + max;
  ...
  private KeycloakGroupTreeRepresentation fillChildrenRecursive(
      KeycloakGroupTreeRepresentation group) {
  ...
  private List<KeycloakGroupTreeRepresentation> getAllChildren(String groupId) {
    ...
    while (true) {
      List<KeycloakGroupTreeRepresentation> page =
          getChildrenPage(groupId, first, CHILDREN_PAGE_SIZE);
      if (page.isEmpty()) {
        break;
      }
      all.addAll(page);
  ...
  private List<KeycloakGroupTreeRepresentation> getChildrenPage(String groupId, int first,
                                                                int max) {
    String url = sdKeycloakProperties.getBaseUrl()
        + "/admin/realms/" + sdKeycloakProperties.getRealm()
        + "/groups/" + groupId
