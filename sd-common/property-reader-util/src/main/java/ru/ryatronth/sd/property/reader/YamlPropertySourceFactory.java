...
public class YamlPropertySourceFactory implements PropertySourceFactory {
  ...
  public PropertySource<?> createPropertySource(String name, EncodedResource encodedResource)
    ...
    if (loader == null) {
      throw new IllegalStateException(
          "No PropertySourceLoader for resource: " + resource.getDescription());
    }
    ...
    log.info("Loaded config (multi-source): {} -> parts: {}", resource.getDescription(),
        loaded.size());
    return composite;
  }
  ...
  private static String fallbackName(String name, Resource resource) {
    return (name != null && !name.isBlank()) ? name :
        "extra-config [" + resource.getDescription() + "]";
  }
