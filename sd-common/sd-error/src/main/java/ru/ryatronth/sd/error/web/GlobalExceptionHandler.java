...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> handleValidation(MethodArgumentNotValidException ex, HttpServletRequest request) {
        List<ErrorFieldViolation> violations =
            ex.getBindingResult().getFieldErrors().stream().map(this::toViolation).collect(Collectors.toList());
    ...
    private ErrorFieldViolation toViolation(FieldError fe) {
        return new ErrorFieldViolation(fe.getField(), fe.getDefaultMessage() == null ? "Invalid value" : fe.getDefaultMessage());
    }
